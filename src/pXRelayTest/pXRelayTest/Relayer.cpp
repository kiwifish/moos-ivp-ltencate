...
bool Relayer::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p = NewMail.rbegin(); p!=NewMail.rend(); p++) {
    ...
    if(key == m_incoming_var_a || key == m_incoming_var_b) // or incoming variable_var_b 
      m_tally_recd++;
...
{
  if(m_incoming_var_a != "")
    m_Comms.Register(m_incoming_var_a, 0);
  if(m_incoming_var_b != "")
    m_Comms.Register(m_incoming_var_b, 0);
}
...
bool Relayer::Iterate()
{
  ...
  for(i=0; i<amt; i++) {
    m_tally_sent++;
    m_Comms.Notify(m_outgoing_var, (m_tally_sent*10)); 
    //std::cout << m_tally_sent*10 << std::endl; //I feel like I should be able to just +10 this
  }
...
bool Relayer::OnStartUp()
{
  ...
  for(p = sParams.begin();p!=sParams.end();p++) {
    ...
    if(MOOSStrCmp(sVarName, "INCOMING_VAR_A")) {
      if(!strContains(sLine, " "))
	m_incoming_var_a = stripBlankEnds(sLine);
    }
    
    if(MOOSStrCmp(sVarName, "INCOMING_VAR_B")) {
      if(!strContains(sLine, " "))
	m_incoming_var_b = stripBlankEnds(sLine);
    }
